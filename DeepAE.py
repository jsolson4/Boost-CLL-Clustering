# install the import for tensorflow
...
#         self.cost = 200*tf.losses.log_loss(self.recon, self.input_x)
    ...
    def fit(self, X, sess, learning_rate=0.15,
            iteration=200, batch_size=150, init=False,verbose=False):
        assert X.shape[1] == self.dim_list[0]
        if init:
            sess.run(tf.global_variables_initializer())
        sample_size = X.shape[0] # 1
        # changing the above sample size to 1 from X.shape[0]
        # batch size from 50 to 15
        # iteration set to 200 means the model will train in the entire data set 200/140 (~ 1.4 times)
